...
namespace Urho3D
{
    ...
    TransformGizmo::TransformGizmo( Camera* camera ) : TransformGizmo( camera, true, GetMainViewportRect() ) {}

    TransformGizmo::TransformGizmo( Camera* camera, const Rect& viewportRect ) : TransformGizmo( camera, false, viewportRect ) {}

    TransformGizmo::TransformGizmo( Camera* camera, bool isMainViewport, const Rect& viewportRect )
        : camera_( camera ), internalViewMatrix_( camera_->GetView().ToMatrix4().Transpose() ), internalProjMatrix_( camera_->GetProjection().Transpose() ), isMainViewport_( isMainViewport ), viewportRect_( viewportRect )
    {
    }

    ea::optional< Matrix4 > TransformGizmo::ManipulateTransform( Matrix4& transform, TransformGizmoOperation op, TransformGizmoAxes axes, bool local, const Vector3& snap ) const
    {
        ...
        Matrix4 delta;
        ImGuizmo::Manipulate( internalViewMatrix_.Data(), internalProjMatrix_.Data(), operation, mode, &internalTransformMatrix.m00_, &delta.m00_, snap != Vector3::ZERO ? snapVector.Data() : nullptr );
    ...
    ea::optional< Vector3 > TransformGizmo::ManipulatePosition( const Matrix4& transform, TransformGizmoAxes axes, bool local, const Vector3& snap ) const
    {
    ...
    ea::optional< Quaternion > TransformGizmo::ManipulateRotation( const Matrix4& transform, TransformGizmoAxes axes, bool local, float snap ) const
    {
        Matrix4       transformCopy = transform;
        const Vector3 snapVector    = snap * Vector3::ONE;
    ...
    ea::optional< Vector3 > TransformGizmo::ManipulateScale( const Matrix4& transform, TransformGizmoAxes axes, bool local, float snap ) const
    {
        Matrix4       transformCopy = transform;
        const Vector3 snapVector    = snap * Vector3::ONE;
    ...
    TransformNodesGizmo::TransformNodesGizmo( Node* activeNode ) : activeNode_( activeNode ), nodes_{ WeakPtr< Node >{ activeNode } } {}

    bool TransformNodesGizmo::Manipulate( const TransformGizmo& gizmo, TransformGizmoOperation op, TransformGizmoAxes axes, bool local, bool pivoted, const Vector3& snap )
    {
        switch ( op )
        {
            case TransformGizmoOperation::Translate:
                return ManipulatePosition( gizmo, axes, local, pivoted, snap );

            case TransformGizmoOperation::Rotate:
                return ManipulateRotation( gizmo, axes, local, pivoted, snap );

            case TransformGizmoOperation::Scale:
                return ManipulateScale( gizmo, axes, local, pivoted, snap );
    ...
    bool TransformNodesGizmo::ManipulatePosition( const TransformGizmo& gizmo, TransformGizmoAxes axes, bool local, bool pivoted, const Vector3& snap )
    {
    ...
    bool TransformNodesGizmo::ManipulateRotation( const TransformGizmo& gizmo, TransformGizmoAxes axes, bool local, bool pivoted, const Vector3& snap )
    {
    ...
    bool TransformNodesGizmo::ManipulateScale( const TransformGizmo& gizmo, TransformGizmoAxes axes, bool local, bool pivoted, const Vector3& snap )
    {
        const Matrix4 anchorTransform = GetGizmoTransform( pivoted );
